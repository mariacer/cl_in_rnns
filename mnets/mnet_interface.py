...
class MainNetInterface(ABC):
    ...
    Attributes:
        internal_params (torch.nn.ParameterList or None): A list of all
            internally maintained parameters of the main network currently in
            use. If all parameters are assumed to be generated externally, then
            ...
            Note:
                In most cases, the attribute will contain the same set of
                parameter objects as the method
                :meth:`torch.nn.Module.parameters` would return. Though, there
                might be future cases where the programmer wants to hide
                parameters from the optimizer in a task- or time-dependent
                manner.
        weights (torch.nn.ParameterList or None): Same as
            :attr:`internal_params`. The attribute is still provided for legacy
            reasons but might become deprecated in the future. Please always use
            :attr:`internal_params`.
        internal_params_ref (list): A list of integers. Each entry either
            represents an index within attribute :attr:`param_shapes` or is set
            to ``-1``.
        ...
        param_shapes (list): A list of lists of integers. Each list represents
            the shape of a parameter tensor. Note, this attribute is
            independent of the attribute :attr:`internal_params`, it always
            comprises the shapes of all parameter tensors as if the network
            would be stand-alone (i.e., no weights being passed to the
            :meth:`forward` method).
        param_shapes_meta (list): A list of dictionaries. The length of the list
            is equal to the length of the list :attr:`param_shapes` and each
            entry of this list provides meta information to the corresponding
            entry in :attr:`param_shapes`. Each dictionary contains the keys
            ``name``, ``index`` and ``layer``. The key ``name`` is a string and
            refers to the type of weight tensor that the shape corresponds to.
            Possible values are:
        ...
        hyper_shapes (list): A list of lists of integers. Each list represents
        ...
        hyper_shapes_learned (list): A list of lists of integers. Each list
            represents the shape of a weight tensor that has to be passed to the
            :meth:`forward` method during training. If all weights are
            maintained internally, then this attribute will be ``None``.
        hyper_shapes_learned_ref (list): A list of integers. Each entry either
            represents an index within attribute :attr:`param_shapes` or is set
            to ``-1``.
            ...
            .. note::
                ...
                The possibility that entries may be ``-1`` should account for
                unforeseeable flexibility that programmers may need.
        hyper_shapes_distilled (list): A list of lists of integers. This
            attribute is complementary to attribute
            :attr:`hyper_shapes_learned`, which contains shapes of tensors that
            are learned through the hypernetwork. In contrast, this attribute
        ...
        has_bias (bool): Whether layers in this network have bias terms.
        has_fc_out (bool): Whether the output layer of the network is a fully-
            connected layer.
        mask_fc_out (bool): If this attribute is set to ``True``, it is
            implicitly assumed that if :attr:`hyper_shapes_learned` is not
            ``None``, the last two entries of :attr:`hyper_shapes_learned` are
            the weights and biases of the final fully-connected layer.
        ...
        has_linear_out (bool): Is ``True`` if no nonlinearity is applied in the
            output layer.
        num_params (int): The total number of weights in the parameter tensors
            described by the attribute :attr:`param_shapes`.
        num_internal_params (int): The number of internally maintained
            parameters as prescribed by attribute :attr:`internal_params`.
