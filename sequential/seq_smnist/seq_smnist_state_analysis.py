...
#
# @title           :sequential/seq_smnist/seq_smnist_state_analysis.py
# @author          :mc
...
.. code-block:: 
    ...
    python3 seq_smnist_state_analysis.py path/to/results/folder/
...
import train_utils_seq_smnist as ctu
import train_args_seq_smnist
from sequential.ht_analyses.state_space_analysis import run

if __name__=='__main__':

    ### Get default config for current dataset.
    config = train_args_seq_smnist.parse_cmd_arguments(default=True)
    ...
    analysis_kwd = {'complexity_measure': 'ssmnist_seq_len',
                    'complexity_measure_name': 'digits per sequence',
                    'forced_params': [],
