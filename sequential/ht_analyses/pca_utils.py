...
def compute_pca(x, n_pcs=None, n_samples=-1):
    ...
    This function also normalizes the data before applying PCA such that the
    means stored in the pca object are zero, and one can easily apply
...
def compute_kpca(x,  n_pcs=None, n_samples=-1, max_num_samples=2000,
    ...
    pca = KernelPCA(kernel=kernel, n_components=n_pcs, \
...
def select_timesteps(x, timesteps=None, stop_bit=None, max_num_samples=-1):
    ...
    Args:
        x (torch.Tensor): The data. It has dimensions
            ...
            ``[batch_size*num_tasks, num_hidden]``.
        timesteps (str, optional): The description of which timesteps to use.
            If ``None``, all timesteps are used, if ``'input'`` only the
            timesteps before the stop bit are used, if ``'output'`` only the
            timesteps after the stop bit are used, if ``'stop'`` only the stop
            bit timestep is used.
        stop_bit (int, optional): The timestep of the stop bit presentation.
...
def get_expl_var_across_tasks(all_act, n_samples=-1, do_kernel_pca=False,
    ...
    for task_id, act in zip(range(1, num_tasks), all_act[1:]):
        ...
        # Note that since all inputs are identical across all tasks, `acts`
        # represents the hidden activations after having learned all previously
        # learned tasks.
        ...
        order = torch.randperm(acts.shape[0])[:max_num_samples]
        subsequent_tasks_act = acts[order, :]
        ...
        for n_pcs in n_pcs_considered[1:]:
            ...
            # Note, the projection is typically just a projection into another
            # coordinate system (first translating by the mean of task 1 data
            # and then rotating in the coordinate system aligned with the PCs
            # of task 1). Then we just set the component wrt "low variance"
            # axis to zero before projecting back. Note, after translation this
            # following step would be equivalent to
            # `torch.matmul(subsequent_tasks_act, pcs.T[:, :n_pcs])`.
            # Note that pca.components_ has dimensions (n_components,n_features)
            # so we would need to transpose it.
            subsequent_tasks_projected = pca.transform(subsequent_tasks_act)
            if do_kernel_pca:
                ksubsequent_tasks_projected = kpca.transform(\
                    subsequent_tasks_act)

            ### Project back to the original space.
            # Equivalent to
            # `torch.matmul(subsequent_tasks_projected, pcs[:n_pcs, :])`
            # (omitting reverse translation, which also occurs under the hood)
            ...
            if do_kernel_pca:
                ksubsequent_tasks_projected_back = kpca.inverse_transform(\
                    ksubsequent_tasks_projected)

            ### Obtain the explained variances.
            # Compute the residual of the reconstruction.
            residual = subsequent_tasks_act - subsequent_tasks_projected_back
            if do_kernel_pca:
                kresidual = subsequent_tasks_act - \
                    ksubsequent_tasks_projected_back
            # Compute the explained variance.
            ve = 1 - LA.norm(np.array(residual), 'fro')**2 / \
                LA.norm(np.array(subsequent_tasks_act), 'fro')**2
            if do_kernel_pca:
