...
def hnet_args(parser, allowed_nets=['hmlp'], dhmlp_arch='100,100',
    ...
    if 'chunked_hmlp' in allowed_nets or 'chunked_hdeconv' in allowed_nets or \
            'structured_hmlp' in allowed_nets:
        nets = '"chunked_hmlp", "chunked_hdeconv" or "structured_hmlp"'
...
def hypernet_args(parser, dhyper_chunks=11010, dhnet_arch='50,50,50',
    ...
    .. deprecated:: 1.0
        Please use function :func:`hnet_args` instead. As this function creates
        arguments for deprecated hypernetworks.
    ...
    """
    warn('Please use function "utils.cli_args.hnet_args" instead. As this ' +
         'function creates argments for deprecated hypernetworks.',
         DeprecationWarning)
...
def main_net_args(parser, allowed_nets=['fc'], dfc_arch='100,100',
    ...
    for nt in allowed_nets:
        assert(nt in ['fc', 'mlp', 'chunked_mlp', 'simple_rnn'])
...
def check_invalid_argument_usage(args):
    ...
    if bn_used is not None and bn_used and hasattr(args, 'net_type'):
        if args.net_type in ['simple_rnn']:
