...
def calc_fix_target_reg(hnet, task_id, targets=None, dTheta=None, dTembs=None,
    ...
    Args:
        ...
        prev_task_embs (list, optional): If given, ``prev_theta`` but not
            ...
            embeddings (conditional parameters) learned prior to learning the
            current task. It is sufficient to only pass the task embeddings for
            tasks with ID smaller than the current one (only those tasks that
            are regularized).
