...
#
# @title           :hnets/chunked_hyper_model.py
# @author          :ch
...
class ChunkedHyperNetworkHandler(nn.Module, CLHyperNetInterface):
    ...
    .. deprecated:: 1.0
        Please use class :class:`hnets.chunked_mlp_hnet.ChunkedHMLP` instead.
    ...
    def __init__(self, target_shapes, num_tasks, chunk_dim=2586,
                 layers=[50, 100], te_dim=8, no_te_embs=False,
        ...
        warn('Please use class "hnets.chunked_mlp_hnet.ChunkedHMLP" instead.',
             DeprecationWarning)
        ...
        self._hypernet = HyperNetwork([[chunk_dim]], num_tasks, verbose=False,
            layers=layers, te_dim=te_dim, no_te_embs=no_te_embs,
            activation_fn=activation_fn, use_bias=use_bias,
            no_weights=no_weights,
            init_weights=None if init_weights is None else init_weights[1:],
        ...
        if no_weights:
            ...
            if init_weights is not None:
                self._embs.data = init_weights[0]
    ...
    def forward(self, task_id=None, theta=None, dTheta=None, task_emb=None,
        ...
        if theta is None:
            ...
            assert(np.all(np.equal([self._num_chunks, self._ce_dim],
                                   list(theta[0].shape))))
